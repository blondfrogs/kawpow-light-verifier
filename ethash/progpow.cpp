...
mix_array init_mix(uint32_t * seed)
{
    const uint32_t z = fnv1a(fnv_offset_basis, static_cast<uint32_t>(seed[0]));
    const uint32_t w = fnv1a(z, static_cast<uint32_t>(seed[1]));
